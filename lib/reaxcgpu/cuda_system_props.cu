...
{
    HIP_DYNAMIC_SHARED( rvec, xcm)
    HIP_DYNAMIC_SHARED( rvec, vcm)
    HIP_DYNAMIC_SHARED( rvec, amcm)
...
{
    HIP_DYNAMIC_SHARED( rvec, my_xcm)
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int xcm_id = threadIdx.x;
    unsigned int index = 0;
    rvec xcm;
...
{
    HIP_DYNAMIC_SHARED( rvec, my_vcm)
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int index = 0;
    rvec vcm;
    real m;
...
{
    HIP_DYNAMIC_SHARED( rvec, my_amcm)
    unsigned int i = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int index = 0;
    rvec amcm;
    real m;
...
CUDA_GLOBAL void k_center_of_mass( rvec *xcm, rvec *vcm, rvec *amcm, 
        rvec *res_xcm, rvec *res_vcm, rvec *res_amcm, size_t n )
{
    HIP_DYNAMIC_SHARED( rvec, sh_xcm)
    HIP_DYNAMIC_SHARED( rvec, sh_vcm)
    HIP_DYNAMIC_SHARED( rvec, sh_amcm)
...
CUDA_GLOBAL void k_compute_center_mass( single_body_parameters *sbp, 
        ...
        size_t n )
{
    HIP_DYNAMIC_SHARED( real, xx)
    HIP_DYNAMIC_SHARED( real, xy)
    HIP_DYNAMIC_SHARED( real, xz)
    HIP_DYNAMIC_SHARED( real, yy)
    HIP_DYNAMIC_SHARED( real, yz)
    HIP_DYNAMIC_SHARED( real, zz)
...
CUDA_GLOBAL void k_compute_center_mass_xx_xy( single_body_parameters *sbp,
        ...
        size_t n )
{
    HIP_DYNAMIC_SHARED( real, my_results_xx)
    HIP_DYNAMIC_SHARED( real, my_results_xy)
...
CUDA_GLOBAL void k_compute_center_mass_xz_yy( single_body_parameters *sbp,
        ...
        size_t n )
{
    HIP_DYNAMIC_SHARED( real, my_results_xz)
    HIP_DYNAMIC_SHARED( real, my_results_yy)
...
CUDA_GLOBAL void k_compute_center_mass_yz_zz( single_body_parameters *sbp,
        ...
        size_t n )
{
    HIP_DYNAMIC_SHARED( real, my_results_yz)
    HIP_DYNAMIC_SHARED( real, my_results_zz)
...
{
#if defined(__SM_35__)
    HIP_DYNAMIC_SHARED( real, my_sbp)
    unsigned int i;
    int z, offset;
    real sdata;
...
{
#if defined(__SM_35__)
    HIP_DYNAMIC_SHARED( real, my_sbpdot)
    unsigned int i;
    int offset, z;
    real sdata;
    rvec p;
        ...
        storage *workspace, rvec xcm, rvec vcm, rvec amcm )
{
    ...
    hipLaunchKernelGGL(k_center_of_mass_blocks_xcm, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(rvec) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, l_xcm, system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );
    
    hipLaunchKernelGGL(k_reduction_rvec, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec) * control->blocks_pow_2 , 0, l_xcm, l_xcm + control->blocks_pow_2, control->blocks_pow_2 );
    hipDeviceSynchronize( );
    cudaCheckError( );
    copy_host_device( xcm, l_xcm + control->blocks_pow_2,
            sizeof(rvec), hipMemcpyDeviceToHost, "momentum:xcm" );
    
    // vcm
    cuda_memset( workspace->scratch, 0, sizeof(rvec) * (control->blocks_pow_2 + 1), "momentum:tmp" );
    l_vcm = r_scratch;
    
    hipLaunchKernelGGL(k_center_of_mass_blocks_vcm, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(rvec) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, l_vcm, system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );
    
    hipLaunchKernelGGL(k_reduction_rvec, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec) * control->blocks_pow_2 , 0,  l_vcm, l_vcm + control->blocks_pow_2, control->blocks_pow_2 );
    hipDeviceSynchronize( );
    cudaCheckError( );
    copy_host_device( vcm, l_vcm + control->blocks_pow_2, sizeof(rvec),
        hipMemcpyDeviceToHost, "momentum:vcm" );
    
    // amcm
    cuda_memset( workspace->scratch, 0,  sizeof (rvec) * (control->blocks_pow_2 + 1), "momentum:tmp");
    l_amcm = r_scratch;
    
    hipLaunchKernelGGL(k_center_of_mass_blocks_amcm, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(rvec) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, l_amcm, system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );
    
    hipLaunchKernelGGL(k_reduction_rvec, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec) * control->blocks_pow_2 , 0,  l_amcm, l_amcm + control->blocks_pow_2, control->blocks_pow_2 );
    hipDeviceSynchronize( );
    cudaCheckError( );
    copy_host_device( amcm, l_amcm + control->blocks_pow_2, sizeof(rvec),
        hipMemcpyDeviceToHost, "momemtum:amcm" );
    ...
    hipLaunchKernelGGL(k_center_of_mass_blocks, dim3(control->blocks_pow_2), dim3(control->block_size), 3 * sizeof(rvec) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, l_xcm, l_vcm, l_amcm, system->n );
    hipDeviceSynchronize( ); 
    cudaCheckError( ); 
    
    hipLaunchKernelGGL(k_center_of_mass, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec) * 3 * control->blocks_pow_2 , 0,  l_xcm, l_vcm, l_amcm, l_xcm + control->blocks_pow_2, l_vcm + control->blocks_pow_2,
          l_amcm + control->blocks_pow_2, control->blocks_pow_2 );
    hipDeviceSynchronize( ); 
    cudaCheckError( );
    
    copy_host_device( xcm, l_xcm + control->blocks_pow_2, sizeof(rvec),
            hipMemcpyDeviceToHost, "momemtum:xcm" );
    copy_host_device( vcm, l_vcm + control->blocks_pow_2, sizeof(rvec),
            hipMemcpyDeviceToHost, "momentum:vcm" );
    copy_host_device( amcm, l_amcm + control->blocks_pow_2, sizeof(rvec),
            hipMemcpyDeviceToHost,"momentum:amcm" );
#endif
}
        ...
        storage *workspace, real *local_results, rvec my_xcm )
{
    ...
    hipLaunchKernelGGL(k_compute_center_mass_xx_xy, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(real) * 2 * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, partial_results,
         my_xcm[0], my_xcm[1], my_xcm[2], system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_compute_center_mass_xz_yy, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(real) * 2 * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, partial_results,
         my_xcm[0], my_xcm[1], my_xcm[2], system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_compute_center_mass_yz_zz, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(real) * 2 * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, partial_results,
          my_xcm[0], my_xcm[1], my_xcm[2], system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_compute_center_mass_opt, dim3(1), dim3(control->blocks_pow_2), sizeof(real) * 6 * control->blocks_pow_2 , 0,  partial_results, partial_results + (control->blocks_pow_2 * 6), control->blocks_pow_2 );
    hipDeviceSynchronize( );
    cudaCheckError( );

    copy_host_device( local_results, partial_results + 6 * control->blocks_pow_2,
        sizeof(real) * 6, hipMemcpyDeviceToHost, "tensor:local_results" );
    ...
    hipLaunchKernelGGL(k_compute_center_mass, dim3(control->blocks_pow_2), dim3(control->block_size), sizeof(real) * 6 * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, partial_results,
          my_xcm[0], my_xcm[1], my_xcm[2], system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );
...
void Cuda_Generate_Initial_Velocities( reax_system *system, real T )
{
    ...
    hipLaunchKernelGGL(k_generate_initial_velocities, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  system->reax_param.d_sbp, system->d_my_atoms, T, system->n );
}
        ...
        storage *workspace, simulation_data* data, MPI_Comm comm )
{
    ...
    hipLaunchKernelGGL(k_compute_kinetic_energy, dim3(control->blocks), dim3(control->block_size), sizeof(real) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, block_energy, system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_reduction, dim3(1), dim3(control->blocks_pow_2), sizeof(real) * control->blocks_pow_2 , 0,  block_energy, block_energy + control->blocks_pow_2, control->blocks_pow_2 );
    hipDeviceSynchronize( );
    cudaCheckError( );
    ...
    copy_host_device( &data->my_en.e_kin, block_energy + control->blocks_pow_2,
            sizeof(real), hipMemcpyDeviceToHost, "kinetic_energy:tmp" );
        ...
        storage *workspace, simulation_data *data, MPI_Comm comm  )
{
    ...
    hipLaunchKernelGGL(k_compute_total_mass, dim3(control->blocks), dim3(control->block_size), sizeof(real) * control->block_size , 0,  system->reax_param.d_sbp, system->d_my_atoms, block_mass, system->n );
    hipDeviceSynchronize( );
    cudaCheckError( );
    ...
    copy_host_device( &tmp, block_mass + control->blocks_pow_2, sizeof(real), 
            hipMemcpyDeviceToHost, "total_mass:tmp" );
        ...
        storage *workspace, simulation_data* data, mpi_datatypes *mpi_data )
{
    ...
    if ( control->press_mode == 0 || control->press_mode == 2 )
    {
        ...
        hipLaunchKernelGGL(k_compute_pressure, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  system->d_my_atoms, system->d_big_box, rvec_spad,
              system->n );

        hipLaunchKernelGGL(k_reduction_rvec, dim3(blocks_n), dim3(block_size), sizeof(rvec) * block_size , 0,  rvec_spad, rvec_spad + system->n,  system->n );
        hipDeviceSynchronize( );
        cudaCheckError( );

        hipLaunchKernelGGL(k_reduction_rvec, dim3(1), dim3(blocks_pow_2_n), sizeof(rvec) * blocks_pow_2_n , 0,  rvec_spad + system->n, rvec_spad + system->n + blocks_n, blocks_n );
        hipDeviceSynchronize( );
        cudaCheckError( );

        copy_host_device( &int_press, rvec_spad + system->n + blocks_n, sizeof(rvec), 
                hipMemcpyDeviceToHost, "Cuda_Compute_Pressure::d_int_press" );
    }

        ...
        storage *workspace, simulation_data *data, mpi_datatypes *mpi_data )
{
    ...
    copy_host_device( &system->big_box, &system->d_big_box,
            sizeof(simulation_box), hipMemcpyHostToDevice, "Cuda_Scale_Box::simulation_data->big_box" );
    copy_host_device( &system->my_box, &system->d_my_box,
            sizeof(simulation_box), hipMemcpyHostToDevice, "Cuda_Scale_Box::simulation_data->my_box" );
    copy_host_device( &system->my_ext_box, &system->d_my_ext_box,
            sizeof(simulation_box), hipMemcpyHostToDevice, "Cuda_Scale_Box::simulation_data->my_ext_box" );
}

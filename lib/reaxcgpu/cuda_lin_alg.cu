...
#else
    HIP_DYNAMIC_SHARED( rvec2, rvals)
    int thread_id = blockDim.x * blockIdx.x + threadIdx.x;
...
void Cuda_CG_Diagonal_Preconditioner( storage *workspace, rvec2 *b, int n )
{
    ...
    hipLaunchKernelGGL(k_diagonal_preconditioner, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  *workspace, b, n );
        ...
        int n, rvec2 result )
{
    ...
    hipLaunchKernelGGL(k_dual_cg_preconditioner, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  *(workspace->d_workspace), x, alpha[0], alpha[1], n, tmp );

    hipDeviceSynchronize( );
    cudaCheckError( );

    /* reduction to calculate my_dot */
    hipLaunchKernelGGL(k_reduction_rvec2, dim3(blocks), dim3(DEF_BLOCK_SIZE), sizeof(rvec2) * DEF_BLOCK_SIZE , 0,  tmp, tmp + n, n);

    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_reduction_rvec2, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec2) * control->blocks_pow_2 , 0,  tmp + n, tmp + 2*n, blocks);

    hipDeviceSynchronize( );
    cudaCheckError( );

    copy_host_device( result, (tmp + 2*n), sizeof(rvec2),
            hipMemcpyDeviceToHost, "my_dot" );
}
        ...
        rvec2 *arr, int n, rvec2 result )
{
    ...
    hipLaunchKernelGGL(k_norm_rvec2, dim3(blocks), dim3(DEF_BLOCK_SIZE), sizeof(rvec2) * DEF_BLOCK_SIZE , 0, arr, tmp, n, INITIAL);
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_norm_rvec2, dim3(1), dim3(control->blocks_pow_2), sizeof(rvec2) * control->blocks_pow_2 , 0,  tmp, tmp + control->blocks_pow_2, blocks, FINAL );
    hipDeviceSynchronize( );
    cudaCheckError( );
        ...
        rvec2 *a, rvec2 *b, rvec2 result, int n )
{
    ...
    hipLaunchKernelGGL(k_dot_rvec2, dim3(blocks), dim3(DEF_BLOCK_SIZE), sizeof(rvec2) * DEF_BLOCK_SIZE , 0,  a, b, tmp, n );
    hipDeviceSynchronize( );
    cudaCheckError( );

    hipLaunchKernelGGL(k_norm_rvec2, dim3(blocks), dim3(DEF_BLOCK_SIZE), sizeof(rvec2) * control->blocks_pow_2 , 0,  tmp, tmp + control->blocks_pow_2, blocks, FINAL );
...
void Cuda_Vector_Sum_Rvec2(rvec2 *x, rvec2 *a, rvec2 b, rvec2 *c, int n)
{
    ...
    hipLaunchKernelGGL(k_rvec2_pbetad, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  x, a, b[0], b[1], c, n);
...
void Cuda_RvecCopy_From( real *dst, rvec2 *src, int index, int n )
{
    ...
    hipLaunchKernelGGL(k_rvec2_to_real_copy, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  dst, src, index, n);
    hipDeviceSynchronize( );
    cudaCheckError( );
}
...
void Cuda_RvecCopy_To(rvec2 *dst, real *src, int index, int n)
{
    ...
    hipLaunchKernelGGL(k_real_to_rvec2_copy, dim3(blocks), dim3(DEF_BLOCK_SIZE ), 0, 0,  dst, src, index, n);
...
/*#if defined(__SM_35__)
    ...
    hipLaunchKernelGGL(k_dual_matvec_csr, dim3(control->matvec_blocks), dim3(MATVEC_BLOCK_SIZE), sizeof(rvec2) * MATVEC_BLOCK_SIZE , 0,  *H, a, b, n );
    hipDeviceSynchronize( );
    cudaCheckError( );
}
...
//    cudaCheckError( );
    ...
    hipLaunchKernelGGL(k_matvec_csr, dim3(control->matvec_blocks), dim3(MATVEC_BLOCK_SIZE), sizeof(real) * MATVEC_BLOCK_SIZE , 0, *H, a, b, n);
    hipDeviceSynchronize( );
    cudaCheckError( );
}
...
//    N = system->N;
    ...
    copy_host_device( spad, x, sizeof(rvec2) * system->total_cap,
            hipMemcpyDeviceToHost, "Cuda_dual_CG:x:get" );
    ...
    copy_host_device( spad, workspace->d_workspace->q2, sizeof(rvec2) * system->total_cap,
            hipMemcpyDeviceToHost, "Cuda_dual_CG:q2:get" );
    Coll( system, mpi_data, spad, RVEC2_PTR_TYPE, mpi_data->mpi_rvec2 );
    copy_host_device( spad, workspace->d_workspace->q2, sizeof(rvec2) * system->total_cap,
            hipMemcpyHostToDevice,"Cuda_dual_CG:q2:put" );
    ...
    for ( i = 1; i < control->cm_solver_max_iters; ++i )
    {
        copy_host_device( spad, workspace->d_workspace->d2, sizeof(rvec2) * system->total_cap,
                hipMemcpyDeviceToHost, "Cuda_dual_CG::d2:get" );
        Dist( system, mpi_data, spad, RVEC2_PTR_TYPE, mpi_data->mpi_rvec2 );
        copy_host_device( spad, workspace->d_workspace->d2, sizeof(rvec2) * system->total_cap,
                hipMemcpyHostToDevice, "Cuda_dual_CG::d2:put" );
...
//    int j;
    ...
    copy_host_device( spad, x, sizeof(real) * system->total_cap,
            hipMemcpyHostToDevice, "Cuda_CG::x:put" );
    ...
    copy_host_device( spad, workspace->d_workspace->q, sizeof(real) * system->total_cap,
            hipMemcpyDeviceToHost, "Cuda_CG::q:get" );
    Coll( system, mpi_data, spad, REAL_PTR_TYPE, MPI_DOUBLE );

    copy_host_device( spad, workspace->d_workspace->q, sizeof(real) * system->total_cap,
            hipMemcpyHostToDevice, "Cuda_CG::q:put" );
    ...
    for ( i = 1; i < control->cm_solver_max_iters && SQRT(sig_new) / b_norm > tol; ++i )
    {
        ...
        Dist( system, mpi_data, spad, REAL_PTR_TYPE, MPI_DOUBLE );
        copy_host_device( spad, workspace->d_workspace->d, sizeof(real) * system->total_cap,
                hipMemcpyHostToDevice, "Cuda_CG::d:put" );
        ...
        copy_host_device( spad + system->n, workspace->d_workspace->p, sizeof(real) * system->n,
                hipMemcpyDeviceToHost, "Cuda_CG::p:get" );
